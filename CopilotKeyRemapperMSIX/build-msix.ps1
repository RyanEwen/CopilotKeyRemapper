<#
.SYNOPSIS
    Builds the Copilot Key Remapper MSIX package (sideload or Store upload).
...
$mainProj    = Join-Path $repoRoot "CopilotKeyRemapper\CopilotKeyRemapper.csproj"
$keyProj     = Join-Path $repoRoot "CopilotKeyRemapperKey\CopilotKeyRemapperKey.csproj"
$manifestSrc = Join-Path $msixDir  "Package.appxmanifest"
$imagesDir   = Join-Path $msixDir  "Images"
$publicDir   = Join-Path $msixDir  "Public"
$pfxFile     = Join-Path $msixDir  "CopilotKeyRemapper.pfx"

$rid        = if ($Platform -eq "ARM64") { "win-arm64" } else { "win-x64" }
$publishDir = Join-Path $repoRoot "CopilotKeyRemapper\bin\$Platform\$Configuration\net10.0-windows10.0.22000.0\$rid\publish"
$keyPublishDir = Join-Path $repoRoot "CopilotKeyRemapperKey\bin\$Platform\$Configuration\net10.0-windows10.0.22000.0\$rid\publish"
$layoutDir  = Join-Path $msixDir  "bin\msix-layout\$Platform"
$outputDir  = Join-Path $msixDir  "bin\msix-output"
$msixFile   = Join-Path $outputDir "CopilotKeyRemapper-$Platform.msix"
...
if ($NoSign) {
    ...
    $cert = New-SelfSignedCertificate -Type Custom -Subject "CN=CopilotKeyRemapper-Dev" `
        -KeyUsage DigitalSignature -FriendlyName "Copilot Key Remapper Dev" `
        -CertStoreLocation "Cert:\CurrentUser\My" `
        -TextExtension @("2.5.29.37={text}1.3.6.1.5.5.7.3.3")
    $pwd = ConvertTo-SecureString -String "CopilotKeyRemapper" -Force -AsPlainText
    Export-PfxCertificate -Cert $cert -FilePath $pfxFile -Password $pwd | Out-Null
    Export-Certificate -Cert $cert -FilePath (Join-Path $msixDir "CopilotKeyRemapper.cer") | Out-Null
    Write-Host "  Created $pfxFile"
    Write-Host "  Trust it (admin) before installing the MSIX:" -ForegroundColor Cyan
    Write-Host "    Import-Certificate -FilePath '$($msixDir)\CopilotKeyRemapper.cer' -CertStoreLocation Cert:\LocalMachine\TrustedPeople" -ForegroundColor DarkCyan
}

# ── Step 1: Publish (self-contained) ────────────────────────────────────────────
Write-Host "`n=== Publishing CopilotKeyRemapper ($Platform $Configuration) ===" -ForegroundColor Cyan
...
Write-Host "`n=== Publishing CopilotKeyRemapperKey (handler) ===" -ForegroundColor Cyan
...
Copy-Item (Join-Path $keyPublishDir "CopilotKeyRemapperKey.exe") $layoutDir -Force
...
if (-not $NoSign) {
    ...
    $signingPwd = if ([string]::IsNullOrEmpty($TrustedPfxPath)) { "CopilotKeyRemapper" } else { $TrustedPfxPassword }
...
if ($NoSign) {
    ...
    if (-not [string]::IsNullOrEmpty($TrustedPfxPath)) {
        ...
        & $signtool sign /fd SHA256 /a /f $pfxFile /p "CopilotKeyRemapper" $msixFile
    }
...
Write-Host "  Then: Settings > Bluetooth & devices > Keyboard > Customize Copilot key > Custom > Copilot Key Remapper"
